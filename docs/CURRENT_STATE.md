...
0. **Content module CR session times very likely have the same UTC-misparse bug** — `/api/sync/courtreserve` stores `cr_event_sessions.start_time`/`end_time` as `new Date(StartTime).toISOString()` (route.ts ~line 296), i.e. the same `new Date(naive-wall-clock-string)` server-TZ-dependent parse the digest just fixed. Rows synced from Vercel are shifted +5/6h; the Settings → Integrations events panel and `/calendar/campaigns` session tables then render them in org timezone, showing wrong times. Needs the same wall-clock treatment (and a re-sync to heal stored rows). **NOT fixed in the v1.1 digest PR — scope.**
