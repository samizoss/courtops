...
### v1.1 follow-ups (2026-07-21, same-day PR from Sami's feedback)

Branch `fix/digest-v11`, four fixes after Sami reviewed the shipped digest:

1. **CRITICAL tz fix** — digest times were shifted ~5h in prod ("LTP-Monday 6pm" rendered 1:00 PM). Court Reserve `StartTime`/`EndTime` are *naive Chicago wall-clock strings* ("2026-07-27T18:00:00", no zone — verified against prod `weekly_digest_runs.events`); `new Date(raw)` parsed them in the server's local TZ (UTC on Vercel) before re-formatting in America/Chicago. Fix: `parseWallClock` in `src/lib/weekly-digest.ts` reads components straight off the string — no Date, no timezone conversion anywhere in the display path; `dayIndex` comes from the string's date part. Backward compatible: pre-fix stored runs hold the same raw strings and re-render correctly. Tests rewritten to pin wall-clock behavior (suite verified green under both `TZ=UTC` and Chicago); the bug only reproduced under UTC, which is why local testing missed it.
2. **Staff visibility** — Newsletter + Weekly Digest sidebar entries and pages open to staff (view-only: previews, Copy HTML, Download PNG; newsletter shows a "View-only — ask an admin to generate" banner with the Generate button disabled). Generate APIs keep owner/admin 403s; `GET /api/weekly-digest/image` loosened to any org member (read-only). Per Sami: "I don't mind all staff"; per-staff "content" capability noted in code as a future refinement.
3. **Event links in the digest email** — event names link to `https://app.courtreserve.com/Online/Events/Details/{crOrgId}/{eventId}` (white/underline/bold, degrades to plain bold). `DigestEvent.eventId` added (null/absent on old runs → unlinked); `crOrgId` is re-read from `orgs.courtreserve_org_id` by the page at render time (org-level config, not stored per-run — old runs get links for free). Footer: address → Google Maps link; site/Instagram/Facebook links from `JAR_BRAND.club.socials`. PNG stays link-free (static image).
4. **Contrast/a11y** — the email date range was red `#b42033` on blue `#004a8d` = **1.35:1** (computed WCAG; Sami: "red on blue is hard") → now white (**8.88:1**). lightBlue `#65bee5` on that blue is only 4.25:1, so text on blue is white/cream only; red reserved for light/white backgrounds (PNG date pill is red-on-white = 6.57:1, passes). Ratios documented in the template + image route; `lang`/alt/`role="presentation"` verified already present.

### Known issues / follow-ups

0. ~~**Content module CR session times have the same UTC-misparse bug**~~ — ✅ **FIXED** by the content-session-tz PR (see the "CR session times" section below: `crWallClockToInstant`, sync route fix, migration 024 with strict apply-once sequencing).
1. ~~**`/api/cron/availability-reminders` redirected to /login by middleware**~~ — ✅ **CONFIRMED AND FIXED** 2026-07-21 (PR #64): prod probe showed 307 before the fix; middleware now bypasses `/api/cron/*`; post-deploy probe returns 401 on a bad Bearer (route reached). The daily reminder cron was silently non-functional until then.
...
5. **`org_settings` RLS is looser than intended (pre-existing, surfaced by the pre-fill review 2026-07-21)** — migration 002's org-wide `FOR ALL` policy was never dropped, so ANY org member (staff included) can SELECT **and UPDATE/INSERT** `org_settings` directly via the Supabase client — including `cr_api_user`/`cr_api_pass`. Migration 011's owner/admin policies are additional permissive policies (OR'd), not replacements. No CourtOps endpoint echoes the creds, so exposure requires a member crafting their own client call — but a hardening migration should drop the 002 policy and keep member SELECT (needed by the pre-fill endpoint) while scoping writes to owner/admin. Trusted-pilot posture accepted for now.

### Late addition — content-module CR session times FIXED (branch `fix/content-session-tz`)

The weekly-digest timezone investigation exposed the same bug in the content module's storage path, now **FIXED** in this PR:

- **Bug**: Court Reserve's `StartTime`/`EndTime` are naive org-local wall-clock strings (verified against prod: "LTP-Monday 6pm" arrives as `2026-07-20T18:00:00`, no offset). The sync route parsed them with `new Date(...)` on Vercel (UTC), so `cr_event_sessions.start_time`/`end_time` stored wall-clock mislabeled as UTC — instants 5-6h early. Every org-timezone display (Settings → Integrations events panel, `/calendar/campaigns` session tables) rendered them 5-6h off.
- **Fix**: new DST-correct helper `crWallClockToInstant(raw, timeZone)` in `src/lib/cr-time.ts` (13 unit tests incl. spring-forward/fall-back edges); sync route now converts using `orgs.timezone` (fallback `America/Chicago`). Display paths needed no format change, but both sourced timezone from `org_settings.timezone` — a column that **does not exist** (it lives on `orgs`); they silently fell back to Chicago. Now read `orgs.timezone`.
- **⚠️ Migration 024** (`024_fix_cr_session_times.sql`) — one-time correction of existing rows, joined per-org timezone. **Sequencing rule: apply exactly once, immediately after this code deploys, BEFORE any Sync Now runs** — running it on rows written by the fixed code would double-shift them 5-6h late. Apply manually (Management API / MCP); NOT auto-applied.
- **Day-granularity fields checked for the same class**: attendance `DateTime`, transaction dates, `MemberSince`/`CreatedDate` are all naive org-local too (verified against live CR API). On Vercel (UTC) the misparse cancels out at day granularity — wall components are preserved, so stored DATEs were already the correct club-local dates; converting them to real instants and taking the UTC date would have *introduced* next-day flips for evening times (7pm CDT = next-day 00:00Z). Instead `toISODate()` now extracts the wall-clock date textually and `fmt()` uses local date components — identical output on Vercel, and no longer flips late-evening dates on non-UTC servers (local dev on a Central-time machine demonstrably flipped `19:39` to the next date before this fix; regression-tested in `src/lib/courtreserve.test.ts`).
- `first_seen_at` (DB default `now()`), `last_synced_at`, `updated_at` are genuine instants — unaffected.

### Late addition — newsletter pre-fill from Court Reserve (branch `feat/newsletter-prefill`)

- **Newsletter builder now pre-fills events/leagues from Court Reserve** (Sami, 2026-07-21: "consider pre filling the newsletter pieces with CR data and mark include or exclude for them to choose"). New member-read `GET /api/newsletter/cr-events?month=YYYY-MM` pulls the month's CR events live (grouped by `EventId` with distinct-session counts; recurring = ≥3 sessions); a "Load from Court Reserve" include/exclude checklist on `/newsletter` (one-offs pre-checked, recurring series unchecked, league-category events get a parallel League Lineup checklist) applies the checked ones into the existing Event/League rows — re-applying replaces only untouched CR prefills, never manually added or edited rows. Wall-clock display helpers (`parseWallClock`/`formatTimeRange`) moved from `weekly-digest.ts` to `src/lib/cr-time.ts` so client code can share them (re-exported for old imports). Zero changes to the AI contract or generate-route permissions. Pure logic + tests in `src/lib/newsletter-prefill.ts(.test.ts)`.

### Late addition — newsletter v1.2: section toggles, reg-field rework, preload at top (branch `feat/newsletter-toggles`)

Sami's feedback (2026-07-21, with a screenshot of the QA gate blocking on 7 MISSING facts for empty spotlight/staff/coach/reg fields): reg fields felt un-tied to any event, empty sections shouldn't block generation, and the CR preload + month choice should lead the form.

- **Per-section on/off toggles**: `templates/newsletter-skeleton.html` gained `<!-- SECTION: NAME --> … <!-- /SECTION: NAME -->` markers (versioned design change; runtime still never mutates the file) around LEAGUES (incl. nested LEAGUE_REG), EVENTS, CLINICS, ANNOUNCEMENTS, COMMUNITY_IMAGE, COMMUNITY_CARDS (wrapper row) → SPOTLIGHT + STAFF (nested), COACH_QUOTE, AHEAD. Core sections (masthead, hero, At a Glance, sign-off, footer) have no markers and always render. New pure helper `removeSections()` in `src/lib/newsletter.ts` excises OFF regions and strips all markers (throws on unknown names; nested regions swallowed by a parent are skipped).
- **Order of operations is the invariant**: excise OFF sections FIRST → expand LEAGUE_ROWS/EVENT_ROWS only for surviving sections → inject slots → UTM → QA gate on the excised HTML — so the gate never demands facts for OFF sections. Assembly extracted to `assembleNewsletter()` in `src/lib/newsletter-sections.ts` and tested route-level without mocking Anthropic. The model's zod slot schema is built **dynamically** (only ON sections' slots; hero/glance/sign-off always) and the system prompt gains one line listing the OFF sections (rules otherwise verbatim). Facts for OFF sections are omitted from the user message.
- **Reg fields reworked**: global member/daily-player registration fields REMOVED from UI + request + prompt (info lives in CR, wasn't event-tied). `LEAGUE_REG_DATES` is no longer model-written: a single optional "League registration info (one line)" field in the Leagues section is injected VERBATIM (escaped) by code, or the whole template line is excised when blank (nested `LEAGUE_REG` marker).
- **Layout**: month/year picker now at the very top; Load-from-Court-Reserve panel directly beneath with the button labeled for the resolved month ("Load August 2026 events from Court Reserve"); changing month clears the CR checklist and flags applied `fromCr` rows stale (amber badge; manual/edited rows never touched); then notes → hero → toggled section cards in email order. UI defaults: LEAGUES/EVENTS/CLINICS/ANNOUNCEMENTS/AHEAD on; COMMUNITY_IMAGE/SPOTLIGHT/STAFF/COACH_QUOTE off. Toggles survive Regenerate and ship in the POST body (`sections`), defaulting to all-on server-side for older clients.
- Section keys/labels live in `src/lib/newsletter-section-keys.ts` (isomorphic — no `node:fs`) so the client builder imports them without dragging in the template loader. 24 new unit tests (`newsletter.test.ts` removeSections + `newsletter-sections.test.ts` schema/assembly); suite 145 green.
...
- **Phase 2 also shipped same night (PR #50)**: `/settings/content` (channels / pillars / audiences config), channel catalog in `src/lib/content-channels.ts`, CR-events preview panel on Settings → Integrations, migration 021 (applied to prod — The Jar seeded with 5 pillars + 8 audiences; channels deliberately unseeded, admin enables what the club uses). Two-reviewer QA before merge: fixed reorder atomicity/races (moves now normalize the full display_order sequence, one move at a time, reload on partial failure) and server-side UTC time rendering on the events panel (now org-timezone). Viewer write-blocking on the new tables is UI-only — same trusted-pilot posture as the rest of the app.
- **Phase 3 also shipped (PR #53)**: `/calendar/campaigns` list/create/detail — milestones CRUD, CR event linking with searchable picker, auto session tables (org-timezone rendered). `/calendar` redirects to `/content` until Phase 5 flips them. Sidebar Content entry opened to admin + staff (content planning is Maddie's job; confirm with Geneva per 7/2 agenda). Two-reviewer QA: 5 findings fixed pre-merge (link-state ordering vs unique-violation dead end, unsaved-edit discard guards on archive/restore + milestone switching, org-timezone session times, admin sidebar gap).
- **Content v2 next**: Phase 4 — **full implementation plan is prepped** at `docs/superpowers/plans/2026-07-02-content-phase4-planning-flow.md` (draft migration 022 with the status remap, 3-PR build order, decisions D1–D4, and a "feedback to fold in" checklist to fill from the 7/2 meeting before building). ⚠️ Migration 022 is NON-additive (replaces the content_calendar status constraint + remaps rows) and must deploy together with the /content status-button patch — see plan PR 1. Not applied to prod.
- **Tooling gotcha (2026-07-01)**: PowerShell 5.1 `Get-Content`/`Set-Content` mojibakes UTF-8 files without BOM (em-dashes → `â€”`). Never bulk-rewrite docs via PS string replace — use proper file tools. PR #54 shipped a mangled CURRENT_STATE; fixed by restoring from the prior commit.
